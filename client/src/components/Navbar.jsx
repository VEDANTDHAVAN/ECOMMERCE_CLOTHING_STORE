    ...
    <header>
      <nav className={iconActive ? "nav-active" : "flex"}>
       ...
       <NavLink className="btn" to={"/"}>
          ...
          <NavLink onClick={()=> setVisible(false)} className="btn " to={"/admin"}>
            Admin
          </NavLink>
  ...
  return (
    <header>
      <nav className={iconActive ? "nav-active" : "flex"}>
        <h2 className="nav-logo flex gap-4 font-serif">
          <NavLink to={"/"}>
            <img src="./clothesLogo.png" className="w-30 h-20 object-fill" alt="logo" />
            Fabric Fusion
          </NavLink>
        </h2>
        <ul className="nav-links flex gap-6">
      ...
      <div className="flex items-center gap-6">
        <img
          ...
          className="w-5 gap-5 cursor-pointer"
          alt="search"
        />
      </div>
      <div className="group relative">
        <img
          ...
          alt="profile"
          className="w-5 gap-2 cursor-pointer"
        />
        <div className="group-hover:flex hidden absolute dropdown-menu right-0 pt-4">
          <div className="flex flex-col gap-2 w-36 py-3 px-5 bg-slate-100 text-cyan-300 rounded">
            <p className="cursor-pointer hover:text-black">My Profile</p>
      ...
      <NavLink to={"/cart"}>
        <img src={assets.cart_icon} alt="cart" className="w-5 min-w-5" />
        <p className="absolute right-[-3px] bottom-[-7px] w-4 text-center leading-3 bg-black text-white aspect-square rounded-full text-[10px] p-1">
          {getCartCount()}
        </p>
      </NavLink>
    </header>
